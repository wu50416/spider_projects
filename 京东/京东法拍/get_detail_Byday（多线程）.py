...
from analysis_pj import analysis_JD
...
if __name__ == '__main__':
    ...
    print(list_data)
    # thread_arg = [('181212990', '广州资产债权拍卖-普宁市麦利制衣有限公司等69户债权', '揭阳市', '金融资产', '债权'), ('167560250', '广东广信食品有限公司债权及东莞市升辉燃料有限公司债权项下设备抵押权产>生的收益权', '揭阳市', '金融资产', '债权')]
    # get_detail_data(list_data)
    if list_data:
        if len(list_data) > 10:
            create_thread_get_data(list_data, 4)
        else:  # 小于10 但大于0
            get_detail_data(list_data)
    logger_warning.warning(f'================== 所有线程： 运行结束！开始解析数据 ==================')
    analysis_JD.run()